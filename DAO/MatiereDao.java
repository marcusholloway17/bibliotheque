...
public class MatiereDao implements IDao<Matiere>{
    ...
    public List<Matiere> getAll() {
        ...
        try{
            ...
            while(result.next()){
                list.add(new Matiere(result.getInt(1),result.getString(2), result.getBoolean(3)));
            }
        }catch(SQLException e){
            System.out.println(e.getMessage());
        }
    ...
    public Matiere getById(Integer id) {
        ...
        try{
            ...
            ResultSet result = statement.executeQuery();
            while(result.next()){
                matiere = new Matiere(result.getInt(1),result.getString(2), result.getBoolean(3));
            }
        }catch(SQLException e){
            System.out.println(e.getMessage());
        }
    ...
    public List<Matiere> getByTitre(String titre) {
        List<Matiere> list = new ArrayList<>();
        try{
            String sql = "SELECT * FROM matiere where titre LIKE '%"+ titre + "%' AND actif=true";
            statement = connection.prepareStatement(sql);
            ResultSet result = statement.executeQuery();
            while(result.next()){
                list.add(new Matiere(result.getInt(1),result.getString(2), result.getBoolean(3)));

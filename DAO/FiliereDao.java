...
public class FiliereDao implements IDao<Filiere>{
    ...
    public List<Filiere> getAll(){
        ...
        try{
            ...
            while(result.next()){
                list.add(new Filiere(result.getInt(1),result.getString(2), result.getBoolean(3)));
            }
        }catch(SQLException e){
            System.out.println(e.getMessage());
        }
    ...
    public Filiere getById(Integer id) {
        ...
        try{
            ...
            ResultSet result = statement.executeQuery();
            while(result.next()){
                filiere = new Filiere(result.getInt(1),result.getString(2), result.getBoolean(3));
            }
        }catch(SQLException e){
            System.out.println(e.getMessage());
        }
    ...
    public List<Filiere> getByTitre(String titre) {
        List<Filiere> list = new ArrayList<>();
        try{
            String sql = "SELECT * FROM filiere where titre LIKE '%"+ titre + "%' AND actif=true";
            statement = connection.prepareStatement(sql);
            ResultSet result = statement.executeQuery();
            while(result.next()){
                list.add(new Filiere(result.getInt(1),result.getString(2), result.getBoolean(3)));

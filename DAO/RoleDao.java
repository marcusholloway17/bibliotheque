...
public class RoleDao implements IDao<Role>{
    ...
    public List<Role> getAll() {
        ...
        try{
            ...
            while(result.next()){
                list.add(new Role(result.getInt(1),result.getString(2), result.getBoolean(3)));
            }
        }catch(SQLException e){
            System.out.println(e.getMessage());
        }
    ...
    public Role getById(Integer id) {
        ...
        try{
            ...
            ResultSet result = statement.executeQuery();
            while(result.next()){
                role = new Role(result.getInt(1),result.getString(2), result.getBoolean(3));
            }
        }catch(SQLException e){
            System.out.println(e.getMessage());
        }
    ...
    public List<Role> getByTitre(String titre) {
        List<Role> list = new ArrayList<>();
        try{
            String sql = "SELECT * FROM role where titre LIKE '%"+ titre + "%' AND actif=true";
            statement = connection.prepareStatement(sql);
            ResultSet result = statement.executeQuery();
            while(result.next()){
                list.add(new Role(result.getInt(1),result.getString(2), result.getBoolean(3)));

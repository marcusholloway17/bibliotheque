...
public class CategorieCoursDao implements IDao<CategorieCours>{
    ...
    public List<CategorieCours> getAll() {
        ...
        try{
            ...
            while(result.next()){
                list.add(new CategorieCours(result.getInt(1),result.getString(2), result.getBoolean(3)));
            }
        }catch(SQLException e){
            System.out.println(e.getMessage());
        }
    ...
    public CategorieCours getById(Integer id) {
        ...
        try{
            ...
            ResultSet result = statement.executeQuery();
            while(result.next()){
                categorieCours = new CategorieCours(result.getInt(1),result.getString(2), result.getBoolean(3));
            }
        }catch(SQLException e){
            System.out.println(e.getMessage());
        }
    ...
    public List<CategorieCours> getByTitre(String titre) {
        List<CategorieCours> list = new ArrayList<>();
        try{
            String sql = "SELECT * FROM categoriecours where titre LIKE '%"+ titre + "%' AND actif=true";
            statement = connection.prepareStatement(sql);
            ResultSet result = statement.executeQuery();
            while(result.next()){
                list.add(new CategorieCours(result.getInt(1),result.getString(2), result.getBoolean(3)));

...
public class UtilisateurDao implements IDao<Utilisateur> {
  ...
  public boolean create(Utilisateur t) {
    ...
    if (!existingLogin(t.getLogin())) {
      try {
        String sql =
          "INSERT INTO utilisateur(login, password, idRole) values(?, ?, ?)";
        statement = connection.prepareStatement(sql);
        statement.setString(1, t.getLogin());
        statement.setString(
          2,
          BCrypt.hashpw(t.getPassword(), BCrypt.gensalt())
        );
      ...
      while (result.next()) {
        list.add(
          new Utilisateur(
            ...
            result.getString(3),
            result.getBoolean(4),
            roleDao.getById(result.getInt(5))
          )
        );
      }
    } catch (SQLException e) {
  ...
  public boolean update(Utilisateur t) {
    ...
    try {
      String sql =
        "UPDATE utilisateur SET password=?, actif=?, idRole=? WHERE id=?";
      statement = connection.prepareStatement(sql);
  ...
  public Utilisateur login(Utilisateur t) {
    ...
    try {
      String sql =
        "SELECT id, login, password, idRole FROM utilisateur WHERE login=? AND actif=true LIMIT 1";
      statement = connection.prepareStatement(sql);
      statement.setString(1, t.getLogin());
      ResultSet result = statement.executeQuery();
        ...
        if (BCrypt.checkpw(t.getPassword(), result.getString(3))) {
          utilisateur =
            new Utilisateur(
              result.getInt(1),
              result.getString(2),
              roleDao.getById(result.getInt(4))
            );
        }
      }
    } catch (SQLException e) {

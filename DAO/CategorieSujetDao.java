...
public class CategorieSujetDao implements IDao<CategorieSujet>{
    ...
    public List<CategorieSujet> getAll() {
        ...
        try{
            ...
            while(result.next()){
                list.add(new CategorieSujet(result.getInt(1),result.getString(2), result.getBoolean(3)));
            }
        }catch(SQLException e){
            System.out.println(e.getMessage());
        }
    ...
    public CategorieSujet getById(Integer id) {
        ...
        try{
            ...
            ResultSet result = statement.executeQuery();
            while(result.next()){
                categoriesujet = new CategorieSujet(result.getInt(1),result.getString(2), result.getBoolean(3));
            }
        }catch(SQLException e){
            System.out.println(e.getMessage());
        }

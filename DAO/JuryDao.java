...
public class JuryDao implements IDao<Jury> {
  ...
  public boolean create(Jury t) {
    ...
    try {
      String sql =
        "INSERT INTO jury(nom, prenoms, idCategorieJury) values(?, ?, ?)";
      ...
      while (result.next()) {
        list.add(
          new Jury(
            ...
            result.getString(3),
            result.getBoolean(4),
            categorieJuryDao.getById(result.getInt(5))
          )
        );
      }
    } catch (SQLException e) {
  ...
  public boolean update(Jury t) {
    ...
    try {
      String sql =
        "UPDATE jury SET nom=?, prenoms=?, actif=?, idCategorieJury=? WHERE id=?";
      statement = connection.prepareStatement(sql);
      statement.setString(1, t.getNom());
      statement.setString(2, t.getPrenoms());
  ...
  public List<Jury> getByName(String nom) {
    ...
    try {
      String sql =
        "SELECT * FROM jury WHERE actif=true AND nom LIKE %" + nom + "%";
      statement = connection.prepareStatement(sql);
      ResultSet result = statement.executeQuery();
      while (result.next()) {
        list.add(

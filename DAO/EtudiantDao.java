...
public class EtudiantDao implements IDao<Etudiant> {
    ...
    public boolean create(Etudiant t) {
        boolean status = true;
        try{
            String sql = "INSERT INTO etudiant(nom, prenoms, idFiliere) values(?, ?, ?)";
    ...
    public List<Etudiant> getAll() {
        ...
        try{
            ...
            while(result.next()){
                list.add(new Etudiant(result.getInt(1), result.getString(2), result.getString(3), result.getBoolean(4), filiereDao.getById(result.getInt(5))));
            }
        }catch(SQLException e){
            System.out.println(e.getMessage());
        }
    ...
    public boolean update(Etudiant t) {
        boolean status = true;
        try{
            String sql = "UPDATE etudiant SET nom=?, prenoms=?, actif=?, idFiliere=? WHERE id=?";
            statement = connection.prepareStatement(sql);
            statement.setString(1, t.getNom());
            statement.setString(2, t.getPrenoms());
    ...
    public Etudiant getById(Integer id) {
        ...
        try{
            ...
            while(result.next()){
                etudiant = new Etudiant(result.getInt(1), result.getString(2), result.getString(3), result.getBoolean(4), filiereDao.getById(result.getInt(5)));
            }
        }catch(SQLException e){
            System.out.println(e.getMessage());
        }
    ...
    public List<Etudiant> findAll(String nom) {
        List<Etudiant> list = new ArrayList<>();
        try{
            String sql = "SELECT * FROM etudiant WHERE actif=true AND nom LIKE %" + nom + "%";
            statement = connection.prepareStatement(sql);
            ResultSet result = statement.executeQuery();
